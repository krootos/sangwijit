...
<script>
    ...
    jQuery(document).ready(function() {
        ...
        if(document.getElementById("affiliation").value && document.getElementById("affiliation").value !=  'ช่างอิสระ' ){
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                if(data == 'true'){
                    swal({
                        title: 'ผิดพลาด',
                        text: "ไม่สามารถเพิ่มข้อมูลได้ เนื่องจากมีข้อมูลอยู่แล้ว",
                        icon: 'warning',
                        buttons: true,
                        dangerMode: true,
                        buttons: ["ดูข้อมูล", "ปิด"]
                        }).then((willDelete) => {
                        if (!willDelete) {
                            // swal("Your imaginary file is safe!");
                            window.location = "datatechnician.php?iden="+iden;
                        }else{
                            $('#iden').val("");
                            $('#iden').focus();
                        }
...
$('#InputFileImg').change( function () {
  var fileExtension = ['jpg','png','gif','pdf','jpeg','PNG','JPG','JPEG','HEIF'];
...
function disableBtn() {
    document.getElementById("affiliation").value = 'ช่างอิสระ';
...
function enableckb(){
    document.getElementById("radio2").checked = true;
    if(document.getElementById("affiliation").value == 'ช่างอิสระ'){
        document.getElementById("affiliation").value = '';
    }
    document.getElementById("affiliation").readOnly = false;
}
